...
contract SimpleMultiSig is TransactionDecoder {                             
  ...
  mapping (address => uint) public nonces; // (only) mutable state
  ...
  /// @param caller_ The caller address to get the nonce for
  function fullNonce(address caller_) public view returns (uint) {
    return (
      (uint256(uint160(address(this))) + uint256(uint160(caller_))) * 0x100000000 + nonces[caller_]
    );
  }
  ...
  function execute(
    uint8[] calldata sigV_, bytes32[] calldata sigR_, bytes32[] calldata sigS_, bytes calldata tx_ // ~ 3.5k gas
  ) external {
    ...
    require(nn == fullNonce(msg.sender), 'invalid nonce'); // check nonce
  ...
  function validateSignatures(
    uint8[] calldata sigV_, bytes32[] calldata sigR_, bytes32[] calldata sigS_, bytes calldata tx_
  ) private view {
