...
contract SimpleMultiSig is TransactionDecoder {
  ...
  constructor(uint8 threshold_, address[] owners_) public {
  ...
  function execute(uint8[] sigV_, bytes32[] sigR_, bytes32[] sigS_, bytes tx_) external { // ~ 3.5k gas
  ...
  function recycle(address _target) onlySelf external {
  ...
  function validateSignatures(uint8[] sigV_, bytes32[] sigR_, bytes32[] sigS_, bytes tx_) private view {
  ...
  function canCall(address) internal returns (bool) {
    return true;
  }
}
